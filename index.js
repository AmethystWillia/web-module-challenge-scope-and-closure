...
/*Task 1: counterMaker()
  ...
  1. What is the difference between counter1 and counter2?
    • counter1 has its count variable inside its function, counter 2 does not.
...
function scoreboard(getInningcb, inningcb, inningNum) {
  let results = [];
  let awayScore = 0;
  let homeScore = 0;
  let totalAway = 0;
  ...
  for (let i = 0; i < inningNum; i++) {
    ...
    totalAway += awayScore;
    totalHome += homeScore;
    results.push(`Inning ${i+1}: Away: ${awayScore} - Home: ${homeScore}`);
  }
  if (totalAway === totalHome) {
    results.push(`This game will require extra innings: Away ${totalAway} - Home ${totalHome}`);
    return results;
  } else {
    results.push(`Final Score: Away ${totalAway} - Home ${totalHome}`);
    return results;
  }
}
